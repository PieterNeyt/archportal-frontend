...
interface Props {
    benefit: Benefit;
    profileBenefits: Benefit[] | undefined;
    userPoints: number;
...
export function BenefitCard({benefit, profileBenefits, userPoints, onBuy, isBuying}: Props) {
            ...
            case BenefitType.UNIQUE_PROFILE_PICTURE:
                return (
                    <div className="flex flex-col items-center justify-center h-full gap-3">
                        <span className="text-xs text-white/40 uppercase font-bold">Preview</span>
                        <Avatar src={benefit.configuration}
                                className="w-24 h-24 border-2 border-white/10 shadow-lg shadow-primary/20"/>
                    </div>
                );
    ...
    return (
        <Card
            className={`py-0 w-[250px] h-[380px] sm:w-[300px] bg-black/30 backdrop-blur-xl border border-white/10 hover:border-white/20 transition-all duration-300 ${isOwned ? 'opacity-90' : ''}`}>
